<template>
  <div id="nav">
    <div class="left">
      <div class="download" :class="{active:isActive_download}" @click="download(img_data[0][0])"><img src="../assets/img/下载.png"><span>{{isActive_download?'LOADING':'DOWNLOAD'}}</span></div>
      <div class="info">
        <h3>参数信息</h3>
      ...
      <div class="function">
        <div class="black" @click="img_state=1">黑白</div>
        <div class="blur" @click="img_state=2">高斯模糊</div>
        <div class="classic" @click="img_state=3">古典</div>
        <img src="../assets/img/左.png" @click="hue_change(22.5)">
        <div class="color_change" @click="hue_change(90)">色相转换</div>
        <img src="../assets/img/左.png" @click="hue_change(-22.5)" style="transform:rotate(-180deg);">
        <div class="default" @click="default_img">还原</div>
        <div class="set" :class="{active:isActive_setting}" @click="set_wallpaper(img_data[0][0])"><img src="../assets/img/扳手.png">设置桌面壁纸</div>
    ...
    <div class="right">
      ...
      <div class="frame">
        <img id="cover" :class="{toBlack:img_state==1,toBlur:img_state==2,toClassic:img_state==3}" :src="img_data[0][1]">
      </div>
    </div>
...
export default {
  ...
  methods: {
    init(){
      get_data().get('/showimg',{params:{pid:this.$store.state.pid}}).then(res=>{
        console.log(res);
    ...
    hue_change(value){
      ...
      document.getElementById('cover').style.filter = `hue-rotate(${this.hue_value}deg)`
    },
    default_img(){
      this.img_state = 0
    ...
    set_wallpaper(pid){
      ...
      get_data().get('/setwallpaper',{params:{pid}}).then(res=>{
        alert('壁纸设置成功！')
