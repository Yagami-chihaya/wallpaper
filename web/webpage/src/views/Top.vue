<template>
  <div
    ...
    id="nav"
  >
    <a :href="downloadURL" :download="downloadName" id="download"></a>
    <div class="header">
    ...
    <div class="serachBar">
      <div class="serach_category_button">
        <button
          ...
          :key="index"
          :class="{
            active: $store.state.isActive_category.indexOf(index) != -1,
          }"
      ...
      <div class="serach_resolution_button">
        <p @click="resolutionBoxOpen">
          分辨率<img
            :class="{ active: isActive_resolution }"
        ...
        <div class="optionsBox" :class="{ active: isActive_resolution }">
          <div>
            <button
              class="default"
              :class="{ active: isActive_resolution_default1 }"
              @click="defaultChange"
            >
              默认
            </button>
            <span v-for="item in resolution_title" :key="item">{{ item }}</span>
            <div v-for="(item, index) in resolution_list" :key="index">
              <button
                v-for="(item2, index2) in item"
                :key="index2"
                :class="{
                  active:
                    isActive_resolution_button[0] == index &&
                    isActive_resolution_button[1] == index2 &&
                    !isActive_resolution_default1,
                }"
                @click="resolutionChange(index, index2, item2)"
              >
                {{ item2 }}
              </button>
            </div>
      ...
      <div class="serach_date_button">
        ...
        <div class="optionsBox" :class="{ active: isActive_date }">
          <button
            ...
            :key="index"
            :class="{ active: isActive_date_button == index }"
            @click="dateChange(index, item)"
          >
            {{ item }}
          </button>
        </div>
    ...
    <div class="content" ref="content">
      <div v-for="(item, index) in url_list" :key="index" class="page">
        <div class="title">
          <h2>
            page <span>{{ index + 1 }}</span
            >/{{ Math.ceil(pre_length / 24) }}
          </h2>
          <div class="line"></div>
        </div>
        ...
        <div class="imgListBox">
          <div class="imgBox" v-for="(item2, index2) in item" :key="index2">
            <el-image
              :src="item2"
              @click="showPicture(pid_list[index][index2])"
              lazy
            ></el-image>
            <div class="backboard">
              ...
              <img
                class="download"
                src="../assets/img/下载little.png"
                @click="download(pid_list[index][index2])"
              />
            </div>
            <div
              class="choose"
              :class="{
                active: choose_list.indexOf(pid_list[index][index2]) != -1,
              }"
              @click="choose(pid_list[index][index2])"
            ></div>
          </div>
        </div>
      </div>
    ...
    <download-box
      :chooseNum="choose_list.length"
      :class="{
        downloadBoxActive: choose_list.length >= 1 || isShowDownloadBox,
        downloadBoxDeactive: choose_list.length == 0 && !isShowDownloadBox,
      }"
    >
      <template v-slot:function>
        <div class="function" @click="fullselect">
          <div class="chooseBox"><img src="../assets/img/全选.png" /></div>
          全选
        </div>
        <div class="function" @click="reverseSelect">
          <div class="chooseBox"><img src="../assets/img/反选.png" /></div>
          反选
        </div>
        <div class="function" @click="clearSelect">
          <div class="chooseBox"><img src="../assets/img/清空.png" /></div>
          清空
        </div>
        <div class="function" @click="downloadSelect">
          <div class="chooseBox"><img src="../assets/img/下载.png" /></div>
          下载
        </div>
      </template>
    </download-box>
  ...
  methods: {
    categoryChange(index) {
      ...
      if (this.$store.state.isActive_category.indexOf(index) != -1) {
        this.$store.state.isActive_category.splice(
          this.$store.state.isActive_category.indexOf(index),
          1
        );
    ...
    serachRefresh() {
      ...
      if (this.$store.state.current_date == "一天") {
        date = "1d";
      } else if (this.$store.state.current_date == "三天") {
        date = "3d";
      } else if (this.$store.state.current_date == "一周") {
        date = "1w";
      } else if (this.$store.state.current_date == "一个月") {
        date = "1M";
      } else if (this.$store.state.current_date == "三个月") {
        date = "3M";
      } else if (this.$store.state.current_date == "半年") {
        date = "6M";
      } else if (this.$store.state.current_date == "一年") {
        date = "1y";
      }

      get_data()
        .get("showdata", { params: { category, resolution, date },timeout:2000 })
        .then((res) => {
    ...
    choose(pid) {
      if (this.choose_list.indexOf(pid) == -1) {
        ...
        get_data()
          .get("/check_sexy", { params: { pid: pid } })
          .then((res) => {
            ElNotification({
              title: res.data > 1 ? "Error" : "Success",
              message:
                res.data > 1 ? "该图片可能包含不良内容！！" : "此图片安全",
        ...
        return new Promise((resolve, reject) => {
          get_data()
            ...
            .then((res) => {
              ...
              download.href = "http://47.105.40.169/pictures/" + res.data;
              download.download = res.data;
              console.log(download.href, download.download);
              setTimeout(() => {
                document.getElementById("download").click();
